...
def fetchResources(Map config) {
    ...
    try {
        ...
        result.ALB_ARN = sh(
            script: "aws elbv2 describe-load-balancers --names ${config.albName} --query 'LoadBalancers[0].LoadBalancerArn' --output text 2>/dev/null || echo 'None'",
            returnStdout: true
        ).trim()
...
def scaleDownOldEnvironment(Map config) {
    ...
    if (!config.ALB_ARN) {
        ...
        def albArn = sh(
            script: "aws elbv2 describe-load-balancers --names ${config.albName ?: 'blue-green-alb'} --query 'LoadBalancers[0].LoadBalancerArn' --output text",
            returnStdout: true
        ).trim()
