...
def deployToBlueService(Map config) {
    ...
    try {
        ...
        def dockerDir = "${tfDir}/blue-green-deployment/modules/aws/ecs/scripts"
        ...
        def blueTgArn = sh(
            script: "aws elbv2 describe-target-groups --names ${blueTgName} --region ${config.awsRegion} --query 'TargetGroups[0].TargetGroupArn' --output text 2>/dev/null || echo ''",
            returnStdout: true
        ).trim()
        ...
        def albArn = sh(
            script: "aws elbv2 describe-load-balancers --names blue-green-alb --region ${config.awsRegion} --query 'LoadBalancers[0].LoadBalancerArn' --output text",
            returnStdout: true
        ).trim()
        
        def listenerArn = sh(
            script: "aws elbv2 describe-listeners --load-balancer-arn ${albArn} --region ${config.awsRegion} --query 'Listeners[?Port==`80`].ListenerArn' --output text",
            returnStdout: true
        ).trim()
        ...
        def existingRule = sh(
            script: "aws elbv2 describe-rules --listener-arn ${listenerArn} --region ${config.awsRegion} --query \"Rules[?contains(to_string(Conditions[?Field=='path-pattern'].Values[]), '${pathPattern}')].RuleArn\" --output text",
            returnStdout: true
        ).trim()
